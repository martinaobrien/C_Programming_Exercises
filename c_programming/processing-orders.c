...
int main(void) // need to bring in a customer struct
{
	...
	struct ProductStock array[] = { StockA, StockB, StockC }; // ours will be a pointer from the customer struct instead of an array - also need to consider what the customer can afford to buy
	
	struct Shop shop = createAndStockShop(); //reading in the shop - have exact csv file - need to figure this out
	
	double totalCostForCustomer = 0;
	for(int i=0; i < 3; i++){ // need to keep track of what we have in stock - this is the for loop
		struct Product p = array[i].product; 
		double price = find(shop, p.name); //find method to find the price given its name
